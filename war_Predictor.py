...
import os # to import some files from other folders
import wbdata as wb
import datetime
...
def fill_na_2023():
   oriatasocdasami=os.path.join("2023 data","23 filtered.csv")
   samyaro2023=os.path.join("2023 data","world_in23.csv")
   ocdasami=read_csv(oriatasocdasami)
   samyaro=read_csv(samyaro2023)
   for index,row in ocdasami.iterrows():
    if isna(row["2023"]):
     replacement_value = samyaro[(samyaro['Indicator Name'] == row['Indicator Name'])]["2023"]
     ocdasami.at[index, '2023'] = replacement_value.iloc[0]
   ocdasami=ocdasami.fillna(0)
   ocdasami.to_csv(os.path.join("2023 data","na_filtered.csv"),index=False)
